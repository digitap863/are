...
require("./backEnd/smrelatedproductsCon1.php");


if (isset($_GET['prdid'])) {
    $product_id = $_GET['prdid'];

    $query = "SELECT * FROM `supermarket` WHERE `id` = '$product_id'";
    ...
    <section class="sub-prdts">
      ...
      <div class="subPrdContents">
        ...
        <div class="swiper rltd_prd_slider">
          <div class="swiper-wrapper">
          <?php
                $related_query = "SELECT * FROM `smrelatedproducts` WHERE `relationid` = '$product_id' LIMIT 15";
                $related_result = mysqli_query($conn, $related_query);
                $fetchprd_src2 = FETCHPRD_SRC2;
                ...
                while ($related_product = mysqli_fetch_assoc($related_result)) {
                  ...
                  echo '<a href="singleProduct.php?prdid=' . $related_product['id'] . '">';
                  echo '<img class="img-fluid" src="' . $fetchprd_src2 . $related_product['image'] . '" alt="image" />';
                   echo '</a>';
                  echo '<h3>' . $related_product['name'] . '</h3>';
                  echo '<h5>' . $related_product['description'] . '</h5>';
                  echo '<a href="singleProduct.php?prdid=' . $related_product['id'] . '">';
                  echo '<button>Know More</button>';
                   echo '</a>';
                  echo '</div>';
                  echo '</div>';
                }
