...
if (isset($_GET['prdid'])) {
    ...
    $query = "SELECT * FROM `refridgerationcabinets` WHERE `id` = '$product_id'";
...
<html lang="en">
  <head>
    ...
    <title>ARE Single Product</title>
  ...
  <body>
          ...
          <div class="swiper bannerSlide">
            <div class="swiper-wrapper">
              <div class="swiper-slide">
                 <?php 
              if (isset($product)) {
                echo '<img class="img-fluid" src="' . $fetchprd_src.$product['image'] . '" alt="Product Image" />';
    ...
    <section class="prd-slider">
      <div class="prd-slider-contents">
        ...
        ?>
            <!-- <h2>Product Name</h2>                      
              <p>
                Our success relies on sustaining consistent growth through
                acquiring and retaining customers by delivering the highest level
                of customer service in conjunction with providing practical and
                economically feasible solutions that solve their problems Our
                success relies on sustaining consistent growth through acquiring
                and retaining customers by delivering the highest level of
                customer service in conjunction with providing practical and
                economically feasible solutions that solve their problems
              </p> -->
          </div>
          <div class="prd-rit">
          <?php
        if (isset($product)) {
            echo '<img class="img-fluid" src="' . $fetchprd_src.$product['image'] . '" alt="" />';
        }
        ?>
          </div>
        </div>
    ...
    <!--sub Product Slidre Starts -->
    <section class="sub-prdts">
      <h2>Related Products</h2>
      <div class="subPrdContents">
        <div class="lft-arrow swiper-nav-rltdPrd-prv">
          <i class="fa-solid fa-arrow-left"></i>
        </div>
        <div class="swiper rltd_prd_slider">
          <div class="swiper-wrapper">
          <?php
                $related_query = "SELECT * FROM `refridgerationcabinets` WHERE `id` != '$product_id' LIMIT 15";
                $related_result = mysqli_query($conn, $related_query);
                
                while ($related_product = mysqli_fetch_assoc($related_result)) {
                  echo '<div class="swiper-slide">';
                  echo '<div class="single-rltd-prd">';
                  echo '<a href="refridgerationcabinetsSingle.php?prdid=' . $related_product['id'] . '">';
                  echo '<img class="img-fluid" src="' . $fetchprd_src . $related_product['image'] . '" alt="" />';
                  echo '</a>';
                  echo '<h3>' . $related_product['name'] . '</h3>';
                  echo '</div>';
                  echo '</div>';
                }
            ?>
        ...
        <div class="rit-arrow swiper-nav-rltdPrd-nxt">
          <i class="fa-solid fa-arrow-right"></i>
        </div>
      </div>
      <div class="sliderNavDots">
        <div class="singleDot swiper-nav-rltdPrd-prv"></div>
        <div class="centerDot"></div>
        <div class="singleDot swiper-nav-rltdPrd-nxt"></div>
      </div>
    </section>
    <!--sub Product Slidre Starts -->
    ...
    <script src="./js/relatedPrdSlidr.js"></script>
